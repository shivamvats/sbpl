...
bool MRMHAPlanner::time_limit_reached() const
{
...
MHASearchState* MRMHAPlanner::get_state(int state_id)
{
...
void MRMHAPlanner::reinit_state(MHASearchState* state)
{
...
void MRMHAPlanner::reinit_search()
{
    clear_open_lists();
}

void MRMHAPlanner::clear_open_lists()
{
    for (int i = 0; i < num_heuristics(); ++i) {
...
int MRMHAPlanner::compute_key(MHASearchState* state, int hidx)
{
...
int MRMHAPlanner::compute_heuristic(int state_id, int hidx)
{
    if (hidx == 0) {
...
int MRMHAPlanner::get_minf(CHeap& pq) const
{
    return pq.getminkeyheap().key[0];
}

void MRMHAPlanner::insert_or_update(MHASearchState* state, int hidx, int f)
{
    CKey new_key;
    new_key.key[0] = f;
...
void MRMHAPlanner::extract_path(std::vector<int>* solution_path, int* solcost)
{
...
bool MRMHAPlanner::closed_in_anc_search(MHASearchState* state) const
{
    return state->closed_in_anc;
}

bool MRMHAPlanner::closed_in_add_search(MHASearchState* state) const
{
    return state->closed_in_add;
}

bool MRMHAPlanner::closed_in_any_search(MHASearchState* state) const
{
